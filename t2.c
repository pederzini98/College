...
int sistemaTriangularSuperior(int n, double a[][MAX], double b[], double x[])
{
   int i;
   double s = 0;
   x[n] = b[n] / a[n][n];
   for (i = n - 1; i >= 0; i--)
   {
      x[i] = (b[i] - sum(a, b, x, i, n, 0)) / a[i][i];
      //printf("\nx %.3f b %.3f sum %.3f a %.3f\n", x[i], b[i], sum(a, b, x, i, n, 0), a[i][i]);
   }
   //printf("\n1  %.3f 2  %.3f 3  %.3f 4  %.3f\n", x[0], x[1], x[2], x[3]);
}

int sistemaTriangularInferior(int n, double a[][MAX], double b[], double x[])
{
   int i;
   double s = 0;
   x[0] = b[0] / a[0][0];
   for (i = 1; i < n; i++)
   {
      x[i] = (b[i] - sum(a, b, x, i, 0, 1)) / a[i][i];
      //printf("\nx %.3f b %.3f sum %.3f a %.3f\n", x[i], b[i], sum(a, b, x, i, n, 0), a[i][i]);
   }
   //printf("\n1  %.3f 2  %.3f 3  %.3f 4  %.3f\n", x[0], x[1], x[2], x[3]);
}

int decomposicaoLU(int n, double a[][MAX], double b[], double x[])
{
...
int gaussCompacto(int n, double a[][MAX], double b[], double x[])
{
   int i = 0, ju = 0, il = 0, t = 1, j = 0, k = 0;
...
int verificaSimetria(int n, double a[][MAX])
{
   int i, j;
   for (i = 0; i < n; i++)
   {
...
int cholesky(int n, double a[][MAX], double b[], double x[])
{
   int i, j, t = 1;
   double mLInversa[MAX][MAX] = {0}, mL[MAX][MAX] = {0}, resultado[MAX];
   // Verifica simetria
   if (verificaSimetria(n, a) == 0)
      return 0;
   // Verifica se é o det é diferente de 0
   for (i = 1; i <= n; i++)
   {
   ...
   for (i = 0; i < n; i++)
   {
      for (j = i; j < n; j++)
      {
         if (i == j)
         {
            mL[i][i] = sqrt((a[i][i] - sumCholesk(n, i, j, mL, 1)));
            //printf("\nL %.3f sq %.3f a %.3f sum %.3f II %d JJ %d\n", mL[i][i], sqrt((a[i][i] - sumCholesk(n, i, j, mL, 1))), a[i][i], sumCholesk(n, i, j, mL, 1), i, j);
         }
         else
         {
            mL[j][i] = (a[j][i] - sumCholesk(n, j, i, mL, 0)) / mL[i][i];
            // printf("\nL2 %.3f a %.3f sum %.3f II %d JJ %d\n", mL[i][i], a[j][i], sumCholesk(n, j, i, mL, 0), i, j);
         }
      }
   }
...
int gaussJordan(int n, double a[][MAX], double b[], double x[])
{
   int i, j, k;
...
int criterioLinha(int n, double a[][MAX])
{
   int i, j;
...
int criterioColuna(int n, double a[][MAX])
{
   int i, j;
   double resultado[MAX];
   for (j = 0; j < n; j++)
...
int cp(int n, double resultado[], double aproxInicial[], double e)
{
   int i;
   ...
   return (maiorDiferenca / maiorValor) < e ? 1 : 0;
}

int jacobi(int n, double a[][MAX], double b[], double e, double aproxInicial[], int maxIte, double x[], int *ite)
{
   int i, j;
   double valorX[MAX] = {0};
...
int criterioSassenfield(int n, double a[][MAX])
{
   int i, j;
   double b[MAX];
   for (i = 0; i < n; i++)
...
int gaussSeidel(int n, double a[][MAX], double b[], double e, double aproxInicial[], int maxIte, double x[], int *ite)
{
   int i, j, k;
   // Verifica se a diagonal principal é diferente de zero
   for (i = 0; i < n; i++)
...
int matrizInversa(int n, double a[][MAX], double rx[][MAX])
{
   int i, j, t;
