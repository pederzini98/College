...
double coefDeterminacao(int n, double tabela[][MAX], double vetorY[])
{
    ...
    for (i = 0; i < n; i++)
    {
        quadrado += pow(tabela[1][i], 2.0); // calculo do somatorio do QUADRADO DE Y;
    }
...
{
    double xy = 0, x = 0, y = 0, x2 = 0; //somatorios
    int i, j;
...
//retorna true se todas as submatrizes principais tem determinante diferente de 0
...
bool decomposicaoLU(int n, double a[][MAX], double b[], double x[])
{
    ...
    if (!temSubMatrizesNaoSingulares(n, a))
        return false; //metodo nao converge
...
bool ajustePolinomial(int grauDesejado, int numPontosTabelados, double tabela[][MAX], double vetorA[], double vetorY[], double *coefDet)
{
    ...
    // Calcula o somatorio do X ate o dobro do grau colocado
    ...
    // Calcula o somatorio de X*Y ate o grau desejado
    soma = 0;
    for (j = 0; j < numPontosTabelados; j++)
    {
        soma += tabela[1][j];
...
int main()
{
    ...
    char opSair;
    double tabela[2][MAX];
    // ponto e a variavel onde deseja se conhecer o P(x) interpolado
